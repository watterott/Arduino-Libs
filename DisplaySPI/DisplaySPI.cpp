...
void DisplaySPI::begin(uint_least8_t clock_div, uint_least8_t rst_pin)
{
  ...
  CS_ENABLE();
  SPI.transfer(0x00); //NOP and stop calibration
  SPI.transfer(0x00); //NOP and stop calibration
  SPI.transfer(0x00); //NOP and stop calibration
  CS_DISABLE();
...
int_least16_t DisplaySPI::drawText(int_least16_t x, int_least16_t y, const char *s, uint_least16_t color, uint_least16_t bg, uint_least8_t size)
{
  uint_least16_t len;
  int_least16_t pos=x;
